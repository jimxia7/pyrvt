...
def check_write_events(ext):
    ...
    pyrvt.tools.write_events(dst_fname, periods, "Period (s)", "psa", "SA (g)", events)
    os.close(handle)
...
def test_operation_psa2fa(fixed_spacing):
    ...
    pyrvt.tools.operation_psa2fa(src_fname, dest_dirname, 0.05, "LP99", fixed_spacing)
...
def test_operation_fa2psa(fixed_spacing):
    ...
    # Do not need to check the output as it is checked in
    # test_calc_compatible_spectra
    pyrvt.tools.operation_fa2psa(src_fname, dest_dirname, 0.05, "LP99", fixed_spacing)
    shutil.rmtree(dest_dirname)
